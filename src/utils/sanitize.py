...
def has_probable_infinite_loop(code: str) -> bool:
    try:
        tree = ast.parse(code)
    except SyntaxError:
        return False
    ...
    class LoopAnalyzer(ast.NodeVisitor):
        def __init__(self):
            self.found_infinite_loop = False
            self.has_blocking_calls = False
        ...
        def visit_While(self, node):
            ...
            if isinstance(node.test, ast.Constant) and node.test.value == True:
                ...
                if not has_break:
                    self.found_infinite_loop = True
            
            # Check for loops with no changing variables
            loop_vars = set()
        ...
        def visit_For(self, node):
            # Check if iterating over a literal/constant
        ...
        def visit_Call(self, node):
            ...
            if isinstance(node.func, ast.Name):
                blocking_funcs = {'input', 'raw_input', 'stdin.read', 'stdin.readline'}
                if node.func.id in blocking_funcs:
                    self.has_blocking_calls = True
            self.generic_visit(node)

        def visit_Attribute(self, node):
            # Check for blocking method calls
            if isinstance(node.value, ast.Name):
                blocking_methods = {'read', 'readline', 'accept'}
                if node.attr in blocking_methods:
                    self.has_blocking_calls = True
            self.generic_visit(node)

    analyzer = LoopAnalyzer()
    analyzer.visit(tree)
    return analyzer.found_infinite_loop or analyzer.has_blocking_calls